...
pub async fn load_describer(url: &str) -> Result<(Box<dyn SqlSchemaDescriberBackend>, ConnectionInfo), SqlError> {
    let wrapper_fut = async {
        ...
        connection.query_raw("SELECT 1", vec![]).await?;
        Result::Ok::<_, SqlError>(connection)
    };

    let wrapper = match async_std::future::timeout(CONNECTION_TIMEOUT, wrapper_fut).await {
        Ok(result) => result?,
        Err(_elapsed) => return Err(SqlError::from(ErrorKind::ConnectTimeout("Runtime timer".into()))),
    };
